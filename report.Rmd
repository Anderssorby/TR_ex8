...
```
\abstract{
We analyze a timeseries of the number of people flying every month from 1949-1961. We need to account for seasonality, slope and local level to get a good model. For this we are using the SARIMA and State Space models which can incorporate these data features. We then can then compare the performance of the models and their practicality. The SARIMA model was significantly easier to implement and gave a good fit. The State Space model is more general and powerfull and this might be exploited better if one knows the underlying mechanisms better. 
}
...
```{r ss-simulate-fun, eval=TRUE, echo=TRUE}
#simulation
...
```
```{r ss-simulate, eval=FALSE, echo=TRUE}
#simulation
simulations <- 500
...
```
```{r sim-hist,fig.cap="\\label{histogramss} A histogram of the estimated variance of the simulated data."}
