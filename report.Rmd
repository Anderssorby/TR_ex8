...
The state vector $X_t$ is a 13-dimensional and the matricies have the dimension suitable for this form. This means that the there are in total 5 parameters that we need to estimate namely $\sigma_1^2$, $\sigma_2^2$, $\sigma_3^2$, $\sigma_w^2 = W_t$ and $\mu=X_1$. 
...
We are using the Structural model given by the r function StructTS to fit our data. This was significantly easier than other approaches. It is important to choose good initial values for the method to acually find a good fit for the model.
...
tsdiag(fit1)
```
```{r plotSS2, fig.cap="\\label{fig:structfit2} "}
plot(tsSmooth(fit1))
```
...
```{r forecast2, fig.cap="\\label{fig:forecast} The forecasted values of the logged air passenger data for the next twelve months in red, with standard errors in grey"}
